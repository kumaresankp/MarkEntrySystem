...
app.get('/generatepdf', (req, res) => {
    ...
    fs.writeFile('Marks.txt', '', function() { console.log('done') })
...
app.post('/updateSubmit', (req, res) => {
    var data = req.body;
    con.query(`UPDATE ${data.updatesub} set ${data.updatesub} = ${data.mark} where reg = ${data.reg}`, function(err) { if (err) return console.log("Updated!") })
    res.sendFile(__dirname + "\\update.html")
})
